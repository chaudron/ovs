...
#
# Dependencies:
#  This script needs pahole to be installed, and it needs a version that has
#  is equal or newer that the following commit:
#    https://git.kernel.org/pub/scm/devel/pahole/pahole.git/commit/?id=e819d737de88dc1fd6d31091c66f826ead40761e
#
#  It also need the readelf binary to be available.
#
...
def get_ovs_definitions(objects, pahole="pahole", pid="self"):
    ...
    if result is None:
        ...
        #
        error, elf_result = run_program(['readelf', "-n",
                                         "--debug-dump=links", vswitchd])

        if error:
            print("ERROR: Failed 'readelf' on \"{}\"!\n{}".
                  format(vswitchd, re.sub('^', ' ' * 7, elf_result,
                                          flags=re.MULTILINE)))
            sys.exit(-1)
    ...
    if result is None:
        #
        # One final try to find it in .dwz information
        #
        print("INFO: Failed to find debug info in \"{}\"!".format(dbg_file))

        dwz_match = re.search(r"Separate debug info file: (.*dwz.*)",
                              elf_result)

        if dwz_match is not None:
            dbg_file = "/usr/lib/debug/.build-id/{}/{}".format(
                match.group(1)[:2], dwz_match.group(1))
    ...
    #
    result = "#define ATOMIC(TYPE) TYPE\n" + \
