...
struct dpif_offload {
    ...
    struct dpif *dpif; //XXX: Should link to dp_offload??
...
struct dpif_offload_class {
    ...
    int (*open)(const struct dpif_offload_class *,
                struct dpif *, struct dpif_offload **dpif_offload);
    ...
    int (*flow_flush)(const struct dpif_offload *dpif_offload);
...
#define DPIF_OFFLOAD_PORT_FOR_EACH(PORT, PORT_MGR) \
