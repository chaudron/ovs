...
static unsigned int rte_flow_offload_ufid_to_thread_id(const ovs_u128 ufid);
static unsigned int rte_flow_offload_thread_init(void);
void rte_flow_offload_thread_set_thread_nb(unsigned int thread_nb);
...
dp_netdev_offload_init(void)
{
    ...
    unsigned int nb_offload_thread = rte_flow_offload_thread_nb();
    unsigned int tid;
    ...
    for (tid = 0; tid < nb_offload_thread; tid++) {
        ...
        ovs_thread_create("rte_offload", dp_netdev_flow_offload_main, thread);
    }
...
flow_mark_alloc(void)
{
    ...
    if (ovsthread_once_start(&init_once)) {
        ...
        flow_mark_pool = id_fpool_create(rte_flow_offload_thread_nb(),
                                         1, MAX_FLOW_MARK);
...
dp_netdev_offload_flow_enqueue(struct dp_offload_thread_item *item)
{
    ...
    tid = rte_flow_offload_ufid_to_thread_id(flow_offload->flow->mega_ufid);
    ...
    OVS_REQ_WRLOCK(dp->port_rwlock)
{
    ...
    ovs_barrier_init(&barrier, 1 + rte_flow_offload_thread_nb());
...
unsigned int
rte_flow_offload_thread_nb(void)
{
    return offload_thread_nb;
}

void
rte_flow_offload_thread_set_thread_nb(unsigned int thread_nb)
{
    offload_thread_nb = thread_nb;
}

static unsigned int
rte_flow_offload_ufid_to_thread_id(const ovs_u128 ufid)
{
    uint32_t ufid_hash;

    if (rte_flow_offload_thread_nb() == 1) {
        return 0;
    }
    ...
    ufid_hash = hash_words64_inline(
            (const uint64_t [2]){ ufid.u64.lo,
                                  ufid.u64.hi }, 2, 1);
    return ufid_hash % rte_flow_offload_thread_nb();
}
...
rte_flow_offload_thread_init(void)
{
    ...
    if (*rte_flow_offload_thread_id_get() == OVSTHREAD_ID_UNSET) {
        ...
        ovs_assert(id < rte_flow_offload_thread_nb());
