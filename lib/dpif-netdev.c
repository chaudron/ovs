...
static void
offload_flow_del_resume(struct dp_netdev_flow *flow,
                        struct dp_netdev_flow *flow_reference,
                        int error)
{
    ...
    if (error == EINPROGRESS) {
        ...
        VLOG_DBG(
            ...
            UUID_ARGS((struct uuid *)&flow->mega_ufid), in_port, error);
    }
...
static void
offload_flow_del_resume_cb(void *aux, struct dpif_flow_stats *stats OVS_UNUSED,
                           unsigned pmd_id OVS_UNUSED,
                           void *flow_reference,
                           void *previous_flow_reference OVS_UNUSED, int error)
{
    struct dp_netdev_flow *flow = aux;

    offload_flow_del_resume(flow, flow_reference, error);
}
                 ...
                 struct dp_netdev_flow *flow)
{
    ...
    struct dpif_offload_flow_del del = {
        ...
        .stats = NULL,
        .cb_data = {
            .callback = offload_flow_del_resume_cb,
            .callback_aux = flow
        },
    };
...
{
    if (error == EINPROGRESS || error == EOPNOTSUPP) {
        return;
    }
