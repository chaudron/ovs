                                           ...
                                           struct dpif_offload *offload)
{
    ...
    ovs_mutex_lock(&dp_offload->offload_mutex);
    error = dpif_offload_attach_provider_to_dp_offload__(dp_offload, offload);
    ovs_mutex_unlock(&dp_offload->offload_mutex);
    return error;
}
                       ...
                       struct dpif_offload **offload)
{
    ...
    if (!dp_offload || !offload || !dump || dump->error) {
        return false;
    }
...
dpif_offload_get_impl_type_by_class(const char *type)
{
    ...
    /* For backwards compatibility 'dpdk' is the same as the 'rte_flow'. */
    if (!strcmp(type, "dpdk")) {
        type = "rte_flow";
    }
                                   ...
                                   odp_port_t port_no)
{
    ...
    ovs_mutex_lock(&dp_offload->offload_mutex);
    netdev = dpif_offload_get_netdev_by_port_id_(dp_offload, offload, port_no);
    ovs_mutex_unlock(&dp_offload->offload_mutex);
