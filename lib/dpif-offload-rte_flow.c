                                  ...
                                  struct rte_offload_thread_item *item)
{
    ...
    /* Disassociate the mark on error as well, since the main reason for the
     * error is that the flow is no longer in hardware. */
...
struct netdev *
dpif_offload_rte_get_netdev(const struct dpif_offload_rte_flow *offload,
                            odp_port_t port_no)
{
    struct dpif_offload_port_mgr_port *port;
...
dpif_offload_rte_close(struct dpif_offload *dpif_offload)
{
    ...
    if (offload_rte->offload_threads) {
        for (int i = 0; i < offload_rte->offload_thread_count; i++) {
            ...
            mpsc_queue_destroy(&offload_rte->offload_threads[i].queue);
            cmap_destroy(&offload_rte->offload_threads[i].megaflow_to_mark);
        }
    }
    free(offload_rte);
}
