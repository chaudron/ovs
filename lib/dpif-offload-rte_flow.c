                                  ...
                                  bool modify)
{
    ...
    error = netdev_offload_dpdk_flow_put(
        ...
        flow->actions_len, &flow->ufid, mark, flow->orig_in_port,
...
static void
dpif_offload_rte_flush_enqueue(const struct dpif_offload_rte_flow *offload,
                               struct netdev *netdev,
                               struct ovs_barrier *barrier)
{
 ...
 */
void
dpif_offload_rte_flush(struct netdev *netdev)
//XXX:    OVS_REQ_WRLOCK(dp->port_rwlock)
{
    ...
    offload_ = ovsrcu_get(const struct dpif_offload *, &netdev->dpif_offload);

    if (!dpif_offload_is_offload_enabled() || !offload_) {
        netdev_close(netdev);
        return;
    }

    offload = dpif_offload_rte_cast(offload_);
...
{
    //XXX: FIX SO WE CAN OFFLOAD:
    unsigned int offload_thread_count = 0;
    struct rte_offload_thread *offload_threads = NULL;
    ...
    unsigned int nb_thread = offload_thread_count;
    unsigned int tid;
    size_t i;
