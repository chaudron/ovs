...
static void
dpif_offload_dummy_pmd_thread_lifecycle(
    const struct dpif_offload *dpif_offload, bool exit, unsigned core_id,
    int numa_id, dpif_offload_pmd_thread_work_cb **callback, void **ctx)
{
    /* Only do this for the 'dummy' class, not for 'dummy_x'. */
    if (strcmp(dpif_offload_class_type(dpif_offload), "dummy")) {
        *callback = NULL;
        *ctx = NULL;
        return;
    }
